    ...
    if d == 0:
        ...
        e = (
            ...
            * (
                1
                - np.sin(
                    np.pi * (d - 0.5 - (cont_cutoff + 35) / 2) / (35 - cont_cutoff)
                )
            )
        )
    elif d <= 110:
        e = (
            0.5
            * weight1
            * (1 + np.sin(np.pi * (d - 0.5 - (110 + 35) / 2) / (110 - 35)))
        )
    else:
        e = weight1
    e = e - weight1
    ...
    for i in range(dim):
        for j in range(i + 1, dim):
            ...
            for atom in ["P", "C3'", "C1'", "C4", "N1"]:
                ...
                pari_j = j + 1
                if abs(i - j) > 0:
                    cstout.write(form % (atm1, pair_i, atm2, pari_j, tmp_name, weight2))
        ...
        for i in range(dim):
            for j in range(dim):
                if i != j:
                    ...
                    if (
                        max(eval(atm1_resid), eval(atm2_resid), eval(atm3_resid)) + 1
                        > dim
                    ):
                        continue
...
def bond_angle_cst_fun1(theta_npz, cutoff, cstout, splines_tmp_dir, intra_weight=10):
    ...
    for angle in theta_npz:
        ...
        for i in range(dim):
            ...
            atm1_resid, atm2_resid, atm3_resid = res_ids
            if (
                max(eval(atm1_resid), eval(atm2_resid), eval(atm3_resid)) + 1 > dim
                or min(eval(atm1_resid), eval(atm2_resid), eval(atm3_resid)) < 0
            ):
                continue
