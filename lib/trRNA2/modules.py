...
class ResNetBlock(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            out_channel, out_channel, kernel_size, padding=dilation, dilation=dilation
        )
...
class TriUpdate(nn.Module):
    def __init__(
        ...
        self.pair_row_attn = TriangleAttention(
            in_dim=in_dim, dim=int(dim_pair_attn), n_heads=n_heads, wise="row"
        )
        self.pair_col_attn = TriangleAttention(
            in_dim=in_dim, dim=int(dim_pair_attn), n_heads=n_heads, wise="col"
        )
    ...
    def forward(self, z):
        ...
        if z.requires_grad:
            z = (
                z
                + self.ps_dropout_row_layer(checkpoint(pair_row_attn, z))
                + self.conv_stem[2](z)
            )
        else:
        ...
        if z.requires_grad:
            z = (
                z
                + self.ps_dropout_row_layer(checkpoint(pair_col_attn, z))
                + self.conv_stem[3](z)
            )
        else:
...
class SelfAttention(nn.Module):
    def __init__(
        self, dim, dim_pair=None, heads=8, dim_head=64, dropout=0.0, tie_attn_dim=None
    ):
        super().__init__()
    ...
    def forward(
        ...
        if exists(tie_attn_dim):
            ...
            if soft_tied:
                ...
                dots = (
                    torch.einsum("b i h r, b r h i d, b r h j d -> b h i j", w, q, k)
                    * self.scale
                )
            else:
                dots = (
                    torch.einsum("b r h i d, b r h j d -> b h i j", q, k)
                    * self.scale
                    * (tie_attn_dim**-0.5)
                )
...
class MSAAttention(nn.Module):
    def __init__(self, tie_row_attn=False, attn_class=SelfAttention, dim=64, **kwargs):
        super().__init__()
